  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative z-40 p-8 h-full flex justify-between flex-col">
          ...
          <div className="flex items-center justify-between">
            <motion.p
              layoutId={layout ? `daysLeft-${card.daysLeft}` : undefined}
              className="text-base font-semibold text-left [text-wrap:balance] w-fit mt-2 bg-white/90 text-black/70 px-5 rounded-full py-2"
            >
              <span className="font-bold text-black/80">{card.daysLeft}</span>{" "}
              days left
            </motion.p>
            <motion.div
              className="bg-white/90 text-black/70 rounded-full p-2 mt-2"
              onClick={handleOpen}
            >
              <Bookmark />
            </motion.div>
      ...
      <motion.div className="flex justify-between items-center">
        ...
        <motion.p
          layoutId={layout ? `daysLeft-${card.daysLeft}` : undefined}
          className="text-sm font-semibold text-right [text-wrap:balance] w-fit mt-2 text-black/70 py-2"
        >
          <span className="font-bold text-black/80">{parseFloat(card.amountRaised).toFixed(1)} APT tokens </span> {" "}({card.progress}%)
        </motion.p>
      </motion.div>
    </>
  );
